...
object BodyDecoder {
  ...
  val stringDecoder: BodyDecoder[String] = BodyDecoder { case (bytes, ct) => ct match {
    case text: ContentType.TextContent =>
      MIMECharset.asJavaCharset(text.charset.getOrElse(MIMECharset.`UTF-8`)).flatMap { implicit chs =>
    ...
    case other =>
      Attempt.Failure(Err(s"Media Type must be text, but is ${ct.mediaType}"))
  }}
