        ...
        case Some((h, tl)) =>
          ...
          expect match {
            case Left(header) =>
              ...
              if (endOfHeader == 0) go(Left(ByteVector.empty), Stream.chunk(Chunk.byteVector(nh.drop(`\r\n`.size))) ++ tl) //strip any leading crlf on header, as this starts with /r/n
              else if (endOfHeader < 0 && nh.size > maxChunkHeaderSize) Pull.raiseError[F](new Throwable(s"Failed to get Chunk header. Size exceeds max($maxChunkHeaderSize) : ${nh.size} ${nh.take(1000).decodeAscii}"))
              else if (endOfHeader < 0) go(Left(nh), tl)
              else {
                val (hdr,rem) = nh.splitAt(endOfHeader + `\r\n`.size)
                readChunkedHeader(hdr.dropRight(`\r\n`.size)) match {
                  case None => Pull.raiseError[F](new Throwable(s"Failed to parse chunked header : ${hdr.take(1000).decodeAscii}"))
